...
namespace ReportGenerator.BitbucketPipe.Tests.ReportGeneratorTests
{
    public class When_Generating_Report : SpecificationBase
    {
        ...
        [Then]
        public void It_Should_Generate_Report()
        {
            _coverageSummary.Should().NotBeNull();
            _coverageSummary.BranchCoveragePercentage.Should().BeGreaterThan(0);
            _coverageSummary.LineCoveragePercentage.Should().BeGreaterThan(0);
        }
